...
export function ClassesPage({ classes, notify, onDataChange, onSelectClass }: { classes: ClassItem[]; notify: Notify; onDataChange: () => Promise<void>; onSelectClass: (id: number) => void }) {
  ...
  const addStudent = async (studentNo: string, name: string) => {
    if (!studentClass) return
    setBusy(true)
    try { await api(`/classes/${studentClass.id}/students`, json('POST', { studentNo, name })); notify(`${name} 已加入 ${studentClass.name}`); setStudentClass(null); await onDataChange() }
    catch (error) { notify((error as Error).message, 'error') }
    finally { setBusy(false) }
  }
  ...
  return <>
    ...
    <section className="panel table-panel">
      ...
      {classes.length ? <div className="table-scroll"><table><thead><tr><th>班级名称</th><th>年级</th><th>学生</th><th>积分合计</th><th>创建时间</th><th className="cell-action">操作</th></tr></thead><tbody>{classes.map(item => <tr key={item.id}>
        <td><button className="table-link" onClick={() => onSelectClass(item.id)}><span className="class-icon"><School size={15} /></span><strong>{item.name}</strong></button></td><td>{item.grade ? `${item.grade}年级` : '—'}</td><td>{item.studentCount} 人</td><td className={item.totalScore < 0 ? 'score-negative' : ''}>{item.totalScore}</td><td>{item.createdAt.slice(0, 10)}</td><td className="cell-action"><div className="row-actions"><Button variant="outline" size="sm" onClick={() => setImportClass(item)}><Upload size={14} />导入</Button><Dropdown.Root><Dropdown.Trigger asChild><Button variant="ghost" size="icon" aria-label="更多操作"><MoreHorizontal size={17} /></Button></Dropdown.Trigger><Dropdown.Portal><Dropdown.Content className="dropdown-content" align="end" sideOffset={5}><Dropdown.Item className="dropdown-item" onSelect={() => setEditingClass(item)}><Pencil size={14} />修改年级与班号</Dropdown.Item><Dropdown.Item className="dropdown-item" onSelect={() => setRosterClass(item)}><Users size={14} />维护学生名单</Dropdown.Item><Dropdown.Item className="dropdown-item" onSelect={() => setStudentClass(item)}><UserPlus size={14} />手动添加学生</Dropdown.Item><Dropdown.Item className="dropdown-item dropdown-danger" onSelect={() => setDeleting(item)}><Trash2 size={14} />删除班级</Dropdown.Item></Dropdown.Content></Dropdown.Portal></Dropdown.Root></div></td>
    ...
    <StudentDialog target={studentClass} busy={busy} onClose={() => setStudentClass(null)} onSubmit={addStudent} />
...
function StudentDialog({ target, busy, onClose, onSubmit }: { target: ClassItem | null; busy: boolean; onClose: () => void; onSubmit: (studentNo: string, name: string) => Promise<void> }) {
  const [studentNo, setStudentNo] = useState('')
  const [name, setName] = useState('')
  useEffect(() => { if (target) { setStudentNo(''); setName('') } }, [target])
  return <Dialog open={!!target} onOpenChange={open => !open && onClose()} title="手动添加学生" description={`添加到 ${target?.name || ''}`} footer={<><Button variant="outline" onClick={onClose}>取消</Button><Button disabled={busy || !studentNo.trim() || !name.trim()} onClick={() => void onSubmit(studentNo.trim(), name.trim())}>添加学生</Button></>}><div className="form-stack"><div className="field"><label htmlFor="student-no">学号</label><Input id="student-no" value={studentNo} onChange={e => setStudentNo(e.target.value)} placeholder="请输入学号" /></div><div className="field"><label htmlFor="student-name">姓名</label><Input id="student-name" value={name} onChange={e => setName(e.target.value)} placeholder="请输入姓名" /></div></div></Dialog>
}
...
function RosterDialog({ target, onClose, notify, onDataChange }: { target: ClassItem | null; onClose: () => void; notify: Notify; onDataChange: () => Promise<void> }) {
  ...
  useEffect(() => {
    if (!target) return
    api<Student[]>(`/classes/${target.id}/students?sort=student_no`).then(setStudents).catch(error => notify(error.message, 'error'))
  }, [target, notify])
  useEffect(() => { if (editing) { setStudentNo(editing.studentNo); setName(editing.name) } }, [editing])
  const update = async () => {
    if (!editing) return
    setBusy(true)
    try { const next = await api<Student>(`/students/${editing.id}`, json('PATCH', { studentNo: studentNo.trim(), name: name.trim() })); setStudents(current => current.map(item => item.id === next.id ? next : item)); setEditing(null); notify('学生信息已更新'); await onDataChange() }
    catch (error) { notify((error as Error).message, 'error') }
    finally { setBusy(false) }
  }
  const remove = async () => {
  ...
  return <>
    <Dialog open={!!target} onOpenChange={open => !open && onClose()} title={`${target?.name || ''}学生名单`} description={`共 ${students.length} 名学生，可修正误录信息。`} width="wide" footer={<Button variant="outline" onClick={onClose}>完成</Button>}><div className="roster-table"><table><thead><tr><th>学号</th><th>姓名</th><th>当前积分</th><th className="cell-action">操作</th></tr></thead><tbody>{students.map(student => <tr key={student.id}><td className="mono">{student.studentNo}</td><td><strong>{student.name}</strong></td><td className={student.score < 0 ? 'score-negative' : ''}>{student.score}</td><td className="cell-action"><Button variant="ghost" size="icon" aria-label={`编辑${student.name}`} onClick={() => setEditing(student)}><Pencil size={14} /></Button><Button variant="ghost" size="icon" aria-label={`删除${student.name}`} onClick={() => setDeleting(student)}><Trash2 size={14} /></Button></td></tr>)}</tbody></table></div></Dialog>
...
function ImportDialog({ target, onClose, notify, onDone }: { target: ClassItem | null; onClose: () => void; notify: Notify; onDone: () => Promise<void> }) {
  ...
  return <Dialog open={!!target} onOpenChange={open => !open && onClose()} title="导入学生名单" description={`目标班级：${target?.name || ''}`} footer={<><Button variant="outline" onClick={onClose}>取消</Button><Button disabled={!file || busy} onClick={() => void upload()}><Upload size={15} />{busy ? '正在导入' : '开始导入'}</Button></>}><label className={`upload-zone ${file ? 'has-file' : ''}`}><input type="file" accept=".xlsx,.xlsm" onChange={e => setFile(e.target.files?.[0] || null)} /><span className="upload-icon"><FileSpreadsheet size={24} /></span><strong>{file ? file.name : '选择 Excel 文件'}</strong><p>{file ? `${(file.size / 1024).toFixed(1)} KB` : '支持 .xlsx / .xlsm，首个工作表，最多 200 人'}</p><span className="button button-outline button-sm">浏览文件<ArrowRight size={14} /></span></label><div className="import-note"><strong>表格要求</strong><span>推荐第一行为“学号、姓名”。没有表头时默认读取前两列；重复学号会自动跳过。</span></div></Dialog>
}
