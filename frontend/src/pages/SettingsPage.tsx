...
export function SettingsPage({ settings, classes, onChange, notify }: Props) {
  ...
  const restoreBackup = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = ''
    if (!file) return
    if (file.size > 512 * 1024 * 1024) { notify('备份文件不能超过 512MB', 'error'); return }
    if (!window.confirm(`确认用“${file.name}”恢复系统？当前数据会先自动生成一份安全备份。`)) return
  ...
  return <>
    ...
    <div className="settings-grid">
      ...
      <section className="panel admin-card"><div className="panel-header"><div><h2>数据备份与恢复</h2><p>在线生成一致性 SQLite 备份</p></div><DatabaseBackup size={17} /></div><div className="admin-card-body"><div className="admin-action-row"><div><strong>下载完整备份</strong><span>包含账号、名单、积分、考勤、课表、设置和教学网页</span></div><a className="button button-outline" href="/api/admin/backup"><Download size={14} />下载</a></div><div className="admin-action-row"><div><strong>从备份恢复</strong><span>最大 512MB；恢复前自动保留当前数据库，完成后重新登录</span></div><Button variant="outline" disabled={busy} onClick={() => restoreRef.current?.click()}><Upload size={14} />选择文件</Button><input ref={restoreRef} className="hidden-file" type="file" accept=".db,.sqlite,.sqlite3" onChange={event => void restoreBackup(event)} /></div></div></section>
    </div>
